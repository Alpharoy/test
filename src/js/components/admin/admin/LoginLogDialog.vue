<template>
    <el-dialog title="" :visible="elVisible" @close="onClose(false)" @open="onOpen" v-bind="$attrs" width="700px"
        ...
        <el-card-table :list-data="listData" :usePageCurrentChangeFunc="false" @onPageCurrentChange="onPageCurrentChange">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            fetchData() {
                api.admin().get(helpers.convertURL(adminURL.admin_user_log_list, {userUUID: this.uuid}), helpers.buildQuery(this.currentPage, {})).then(response => {
