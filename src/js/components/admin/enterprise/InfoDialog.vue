<template>
    <el-dialog title="企业新增/编辑" :visible="elVisible" @close="onClose(false)" @open="onOpen" v-bind="$attrs"
               width="1200px"
               class="ql-dialog">
        ...
        <div slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="auditPassedFunc" v-if="isAudit && enterprise.can_audit_passed">审核通过
            </el-button>
            <el-button type="danger" @click="auditFailedFunc" v-if="isAudit && enterprise.can_audit_failed">审核不通过
            </el-button>
            <el-button type="warning" @click="reverseAuditFunc" v-if="isAudit && enterprise.can_reverse_audit">撤销审核
            </el-button>
        </div>
    </el-dialog>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            onOpen() {
                if (!this.creating) {
                    api.admin().get(helpers.convertURL(adminURL.get_enterprise, {enterpriseUUID: this.enterpriseUUID})).then((response) => {
            ...
            onClose(refresh = false) {
                ...
                helpers.restoreDataToDefault(this, ['user', 'enterprise', 'businessLicensePhoto']);
            ...
            submitFunc() {
                if (this.creating) {
                    api.admin().post(helpers.convertURL(adminURL.add_enterprise), {
                        enterprise: this.enterprise,
                        user: this.user
                    }).then(() => {
                        this.$message({type: 'success', message: '新增企业成功'});
                        this.onClose(true);
                    });
                } else {
                    api.admin().put(helpers.convertURL(adminURL.update_enterprise, {enterpriseUUID: this.enterpriseUUID}), this.enterprise).then(() => {
                        this.$message({type: 'success', message: '编辑企业成功'});
                        this.onClose(true);
                    });
                }
            },
            ...
            auditPassedFunc() {
                this.$confirm('将审核企业成功，审核完成后不能撤销', '提示', {type: 'warning'}).then(() => {
                    this.auditFunc(99);
                }).catch(() => {
                });
            },
            auditFailedFunc() {
                this.$confirm('将审核企业状态为失败，确定吗？', '提示', {type: 'warning'}).then(() => {
                    this.auditFunc(80);
                }).catch(() => {
                });
            },
            ...
            auditFunc(auditStatus) {
                api.admin().post(helpers.convertURL(adminURL.enterprise_audit_status, {enterpriseUUID: this.enterpriseUUID}), {
                    audit_status: auditStatus
                }).then(() => {
                    this.$message({type: 'success', message: '审核企业完成'});
                    this.onClose(true);
                })
            },
        },
