...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            fetchNode() {
                api.admin().get(helpers.convertURL(adminURL.node_list), {group: this.group}).then((response) => {
            ...
            fetchMenu(menuId) {
                return api.admin().get(helpers.convertURL(adminURL.get_menu, {id: menuId}));
            },
            submitFunc() {
                let nodes = [];
            ...
            submitCreate() {
                ...
                api.admin().post(helpers.convertURL(adminURL.add_menu), this.menu).then(response => {
                    this.$message({type: 'success', message: '菜单创建成功'});
                    this.onClose(true);
                });
            },
            submitUpdate() {
                api.admin().patch(helpers.convertURL(adminURL.update_menu, {id: this.id}), this.menu).then(response => {
