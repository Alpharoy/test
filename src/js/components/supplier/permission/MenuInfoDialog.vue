...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            fetchNode() {
                api.supplier().get(helpers.convertURL(supplierURL.node_list), {group: this.group}).then((response) => {
            ...
            fetchMenu(menuId) {
                return api.supplier().get(helpers.convertURL(supplierURL.get_menu, {id: menuId}));
            },
            submitFunc() {
                let nodes = [];
            ...
            submitCreate() {
                ...
                api.supplier().post(helpers.convertURL(supplierURL.add_menu), this.menu).then(response => {
                    this.$message({type: 'success', message: '菜单创建成功'});
                    this.onClose(true);
                });
            },
            submitUpdate() {
                api.supplier().patch(helpers.convertURL(supplierURL.update_menu, {id: this.id}), this.menu).then(response => {
