<template>
    <el-dialog title="供应商注册" :visible="elVisible" @close="onClose(false)" @open="onOpen" v-bind="$attrs" width="1200px"
...
<script>
    ...
    export default {
        ...
        methods: {
            onOpen() {
                helpers.restoreDataToDefault(this, ['user', 'supplier', 'smsTimer', 'sendSmsTips', 'countDown', 'step', 'agreementChecked', 'loading']);
            },
            ...
            nextStepFunc() {
                this.loading = true;
                api.supplier().post(helpers.convertURL(supplierURL.register_verify_sms_code), this.user).then(() => {
                    this.step = 'two';
                }).finally(() => {
                    this.loading = false;
                });
            ...
            registerFunc() {
                ...
                this.loading = true;
                api.supplier().post(helpers.convertURL(supplierURL.register), this.supplier).then(() => {
                    this.$message({type: 'error', message: '供应商注册成功，请等待审核。审核成功后即可登录。', duration: 0});
                    this.onClose();
                }).finally(() => {
                    this.loading = false;
                });
