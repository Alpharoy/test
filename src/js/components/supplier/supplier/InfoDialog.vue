<template>
    <el-dialog title="供应商资料" :visible="elVisible" @close="onClose(false)" @open="onOpen" v-bind="$attrs" width="1200px"
               class="ql-dialog">
                ...
                <el-row>
                    <el-col :span="8">
                        <el-form-item label="公司名称" required>
                            <el-input :value="supplier.supplier_name" maxlength="32"
                    ...
                    <el-col :span="8">
                        <el-form-item label="统一信用代码" required>
                            <el-input :value="supplier.usci_number" maxlength="32"
...
<script>
    ...
    export default {
        ...
        methods: {
            onOpen() {
                api.supplier().get(helpers.convertURL(supplierURL.get_supplier, {supplierUUID: this.supplierUUID})).then((response) => {
            ...
            submitFunc() {
                api.supplier().put(helpers.convertURL(supplierURL.update_supplier, {supplierUUID: this.supplierUUID}), this.supplier).then(() => {
                    this.$message({type: 'success', message: '供应商信息更新成功'});
                    this.onClose(true);
                });
