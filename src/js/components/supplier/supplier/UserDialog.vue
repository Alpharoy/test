<template>
    <el-dialog title="管理员列表" :visible="elVisible" @close="onClose(false)" @open="onOpen" v-bind="$attrs" width="1200px"
        ...
        <user-info-dialog :visible.sync="showUserInfoDialog" :uuid="dialogUserUUID" @close="closeUserInfoDialogFunc"></user-info-dialog>
    ...
    export default {
        ...
        methods: {
            onOpen() {
                ...
                api.supplier().get(helpers.convertURL(supplierURL.supplier_user_list)).then(response => this.listData = response);
            },
