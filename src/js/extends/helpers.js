...
export default {
    ...
    // 证件照上传后处理， 会改变 certificates 的值
    updateCertificate(code, fileInfo, certificates) {
        let hasUpdate = false;
        _.forIn(certificates, function (value, key) {
            if (parseInt(value.code) === parseInt(code)) {
                hasUpdate = true;
                certificates[key]['image'] = fileInfo['file_code'];
                return false;
            }
        });
        if (!hasUpdate) {
     ...
     * 列数据累加和
     * @param param
     * @param  {Object} conversion //需要数据换算的字段或下标 默认是除法
     * @param  {Object} filed 自定义换算函数例子
     * @param  {function} transfer //自定义换算函数
     * @returns {[]}
     */
    getSummaries(param, conversion = null, filed = null, transfer = null) {
        const {columns, data} = param;
        const sums = [];

        let properties = [];
        if (conversion != null) {
            properties = Object.keys(conversion);
        }
        ...
        columns.forEach((column, index) => {
            ...
            let values = [];
            if (filed != null && typeof filed === 'object' && filed.hasOwnProperty(property) && typeof filed[property] === 'function') {
                values = data.map(item => Number(filed[property](item, column)));
            } else {
                values = data.map(item => Number(_.get(item, column.property, item[column.property])));
            }
            if (!values.every(value => isNaN(value))) {
                ...
                if (transfer) {//可使用定义函数做四则运算
                    sums[index] = typeof transfer === 'function' ? transfer(sums[index], column, index) : sums[index];
                } else if (conversion != null) {
                    sums[index] /= (typeof conversion[property] !== 'undefined' && Number(conversion[property]) !== 0 ? Number(conversion[property]) : 1);
                }
            }
        });
        return sums;
     ...
     * @returns {Object}
     */
    restoreDataToDefault(thisObject, paths = null) {
        const defaultData = thisObject.$options.data.apply(thisObject);
        let assignData = defaultData;
        if (paths) {
            assignData = _.pick(defaultData, paths);
        }

        return Object.assign(thisObject.$data, assignData);
    },
