<template>
    <div class="flex-container">
        <el-row class="top-container">
            <el-col :span="24">
                <el-button type="primary" @click="infoFunc()">创建任务</el-button>
            </el-col>
        </el-row>
        ...
        <el-card-table :list-data="listData" :show-summary="true" :summary-method="getSummaries">
            ...
            <el-table-column fixed="right" label="操作" width="100">
                <template slot-scope="scope">
                    ...
                    <el-button type="text" @click="deleteFunc(scope.row.task_uuid)">删除</el-button>
                </template>
            </el-table-column>
        </el-card-table>
...
<script>
    ...
    import InfoDialog from '../../../components/enterprise/task/InfoDialog';
    ...
    export default {
        ...
        methods: {
            ...
            fetchTask(page, query) {
                api.enterprise().get(this.convertURL(enterpriseURL.task_list), helpers.buildQuery(page, query)).then(response => {
            ...
            deleteFunc(taskUUID) {
                this.$confirm('将删除此任务订单, 是否继续?', '提示', {type: 'warning'}).then(() => {
                    api.enterprise().delete(this.convertURL(enterpriseURL.delete_task, {taskUUID: taskUUID})).then(response => {
                        this.$message({type: 'success', message: '任务订单删除成功'});
                        this.urFetchData();
                    })
                }).catch(err => {
                });
