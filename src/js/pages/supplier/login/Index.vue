<template>
    <div class="login-container">
        ...
        <h1 class="title">OJMS - 供应商管理后台</h1>
        <div class="login-form">
            ...
            <div class="form">
                ...
                <el-form size="big">
                    ...
                    <el-button type="text" style="margin-left: -190px;" @click="showRegisterDialog = true">供应商注册</el-button>
                </el-form>
            </div>
        </div>
        <a v-show="false" href="http://www.miibeian.gov.cn/" target="_blank" class="copyright">粤ICP备XXX号</a>
        <supplier-register-dialog :visible.sync="showRegisterDialog" :token="token"
                                  @close="closeRegisterDialogFunc"></supplier-register-dialog>
    </div>
</template>
<script>
    import api from '../../../api';
    ...
    import SupplierRegisterDialog from '../../../components/supplier/register/SupplierRegisterDialog';

    export default {
        name: 'LoginIndex',
        components: {SupplierRegisterDialog},
        data: () => ({
            loading: true,
            form: {
                account: '',
        ...
        methods: {
            ...
            login() {
                this.loading = true;
                api.supplier().post(helpers.convertURL(supplierURL.login), this.form, {useLoading: false}).then(response => {
